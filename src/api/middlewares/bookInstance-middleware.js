...
/**
 * Get book ID for multiple book instances if book exist
 * @public
 */
exports.getBookMultiple = async function (req, res, next) {
  try {
    const bookInstances = [];
    for (const bookInstance_ of req.body.bookInstances) {
      const bookInstance = omit(bookInstance_, ['book', 'bookId']);
      const { book, bookId } = bookInstance_;
      if (bookId && await Book.findById(bookId)) {
    ...
    res.bookInstances = bookInstances;
  } catch (error) {
    if (error.message.startsWith('Cannot find')) {
      return next(new APIError({
        message: error.message,
...
exports.checkExistence = async function (req, res, next) {
  try {
    const ids = req.body.ids || req.body.bookInstances.map(({ id }) => id);
